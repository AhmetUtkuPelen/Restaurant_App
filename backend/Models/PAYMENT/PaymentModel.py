...
class Payment(Base):
    ...
    provider = Column(String, nullable=True)  # e.g. 'iyzico', 'stripe'
    ...
    conversation_id = Column(String, nullable=True)  # Iyzico conversation ID for tracking
    ...
    card_family = Column(String, nullable=True)  # e.g., 'Bonus', 'Maximum'
    card_association = Column(String, nullable=True)  # e.g., 'VISA', 'MASTER_CARD'
    card_type = Column(String, nullable=True)  # e.g., 'CREDIT_CARD', 'DEBIT_CARD'
    
    # Transaction details
    ip_address = Column(String, nullable=True)  # User's IP (required by Iyzico)
    basket_id = Column(String, nullable=True)  # Reference to order/basket
    
    # payment_metadata can hold items/reservation info; use JSON where supported
    try:
        payment_metadata = Column("metadata", JSON, nullable=True)
    except Exception:
        # fallback for SQLite dialect specifics in some envs
    ...
    # many-to-many relationship to orders paid by this payment
