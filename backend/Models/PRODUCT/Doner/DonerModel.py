...
class Doner(Product):
    ...
    __mapper_args__ = {
        'polymorphic_identity': 'doner',
    }

    id = Column(Integer, ForeignKey('products.id'), primary_key=True)
    size = Column(SAEnum(DonerSize, native_enum=False), nullable=False, default=DonerSize.MEDIUM)
    ...
    def description_summary(self):
        return f"{self.meat_type.value.title()} doner with {self.spice_level.value.lower()} spices."

    def __repr__(self):
        return f"<Doner(name={self.name}, meat_type={self.meat_type}, spice={self.spice_level})>"
